...
### KV Binding Not Applied to Pages Project

The most common issue with our deployment is that KV namespace bindings specified in `wrangler.toml` are not automatically applied to Pages deployments. This results in the error "KV namespace INVITES is not configured" when trying to access invite links.

**Symptoms:**
- KV-dependent features fail with error messages about missing KV namespaces
- Code that checks for `env.INVITES` displays "KV namespace not configured" errors
- Pages deployment works, but specific endpoints return 500 errors

**Fix:**
1. Add KV bindings manually in the Cloudflare Dashboard:
   - Go to **Pages** > **[Project Name]** > **Settings** > **Functions** > **KV namespace bindings**
   - Add binding with the correct name (e.g., `INVITES`) and select the appropriate namespace

**Important:** Unlike with Workers, KV bindings for Pages projects can *only* be set through the Cloudflare Dashboard, not via the Wrangler CLI.

**Prevention:**
- Add binding verification steps to your post-deployment checklist
- Create test endpoints that can function without KV (like our `/create-test-invite/` endpoint)
- Document all required bindings for each project
- Add monitoring for KV binding health
