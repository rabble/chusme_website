...
import { getRouteHandler } from '../lib/router';
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (path === '/' || path === '/index.html') return indexPage(request);
    ...
    if (path === '/use-holis' || path === '/use-holis') return useHolisApp(request);
    ...
    if (path === '/use-cases/artists') return artistsPage(request);
